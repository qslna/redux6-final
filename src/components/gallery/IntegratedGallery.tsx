...
interface IntegratedGalleryProps {
  /** 갤러리 폴더 경로 */
  folder: string;
  /** 갤러리 레이아웃 */
...
export default function IntegratedGallery({
  folder,
  layout = 'grid',
  maxItems,
  title,
      ...
      const mediaItems: MediaItem[] = data
        ...
        .map((file: any) => {
          ...
          return {
            ...
            thumbnail: isVideo ? `${file.url}?tr=f-jpg,q-80` : file.url
          };
        });
  ...
  const handleDelete = async (fileId: string, itemName: string) => {
    if (!isAdmin || !confirm(`'${itemName}'을(를) 삭제하시겠습니까?`)) return;
    ...
    return (
      <div className={className}>
            ...
            >
              {uploading ? (
                ...
                <>
                  ...
                  <span className="text-sm">미디어 추가</span>
                </>
              )}
            </button>
          </div>
        ...
        >
            ...
            {items.length === 0 ? (
              ...
              >
                <div className="text-center text-white/50">
                  ...
                  {isAdmin && <p className="text-xs mt-1">위의 버튼을 클릭하여 추가하세요</p>}
                </div>
              </motion.div>
            ) : (
          ...
          {href && items.length > 0 && (
            <motion.div 
              className="absolute inset-0 bg-black/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none"
              style={{ zIndex: 1000 }}
            >
  ...
  return (
    <div className={className}>
      ...
      {/* 업로드 진행 상태 */}
      {uploading && (
        <div className="fixed bottom-4 right-4 bg-black/70 backdrop-blur-sm rounded-lg p-4 text-white">
          <div className="flex items-center gap-3">
            <Upload size={16} />
            <div className="flex-1">
              <div className="text-sm mb-1">업로드 중... {Math.round(uploadProgress)}%</div>
              <div className="w-48 bg-gray-600 rounded-full h-1">
                <div 
